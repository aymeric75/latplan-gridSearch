...
"""
    def __init__(self,path,parameters={}):
        ...
        if parameters:
            ...
            self.path = "samples/blocks_cylinders-4-flat_20000_CubeSpaceAE_AMA4Conv_kltune2/grid1/logs/06-17T08:42:16.988"
...
Poor python coders cannot enjoy the cleanness of CLOS :before, :after, :around methods."""
    ...
    def bar_update(self, epoch, logs):
        ...
        if (epoch % 10) == 9:
            ...
            self.bar.update(epoch+1, status = "[v] "+os.path.basename(os.path.normpath(self.path))+" "+"  ".join(["{} {:8.3g}".format(k,v) for k,v in sorted(vlogs.items())]) + "\n")
        else:
 ...
 This method may be overloaded by the subclass into a specific training method, e.g. GAN training."""
        ...
        shutil.copytree("problem-generators/backup-propositional/vanilla/prob-cylinders-4/007-001", self.path+"/007-001")
                ...
                for train_subdata_cache,train_subdata_to_cache in zip(train_data_cache,train_data_to_cache):
                    for net,train_subdata_batch_cache,train_subdata_to_batch_cache in zip(self.nets, train_subdata_cache,train_subdata_to_cache):
                        ...
                        if epoch % 10 == 0 and epoch > 0 and some_count == 0:
                            ...
                            ## avant chaque train, mettre le  "problem-generators/backup-propositional/vanilla/prob-cylinders-4/007-001"
                            ## dans le self.path

                            # generates PROBLEM
                            from latplan.util.planner import puzzle_module, init_goal_misc, setup_planner_utils
                            problem_dir=self.path+"/007-001"
                            network_dir=self.path
                            setup_planner_utils(self, problem_dir, network_dir, "ama3")
                            
                            p = puzzle_module(self)
                            self.parameters["generator"] = "latplan.puzzles.blocks"
