...
from latplan.util.tuning import simple_genetic_search, grid_search, parameters, nn_task, reproduce
...
def main(parameters={}):
    ...
    params['aeclass'] = vars(args)['aeclass']
...
def run(path,transitions,extra=None):
    ...
    if 'dump' in args.mode:
        ...
        if(args.hash != ""):
            path_to_json = os.path.join(path+"/grid1/logs/"+args.hash)
    ...
    if 'learn' in args.mode:
        ...
        path_to_json = os.path.join(path+"/grid1/logs/06-17T08:42:16.988")

        print("path_to_jsonpath_to_json")
        print(path_to_json)
    ...
    #     print("pattthhhh")
    #     print(path)
    #     print(os.path.join(path,"/aux.json"))
        
    #     # samples/blocks_cylinders-4-flat_20000_CubeSpaceAE_AMA4Conv_kltune2
    #     import json
    #     with open("samples/blocks_cylinders-4-flat_20000_CubeSpaceAE_AMA4Conv_kltune2/grid1/aux.json","r") as f:
    #         pars = json.load(f)["parameters"]
    #         parameters["mean"] = pars["mean"]
    #         parameters["std"] = pars["std"]


    #     path = path+"/grid1"
    #     print(path)

    #     parameters['beta_z'] = [10] # grid2 = 1, grid1 = 10
